...
export default function MinisterApprovalDetail({ params }: { params: { id: string } }) {
    ...
    const handleDecision = async (decision: 'APPROVE' | 'REJECT') => {
        if (!notes.trim()) {
            alert('A ministerial decision requires official notes. Please provide them before submitting.');
            return;
        }
        if (!confirm(`You are about to ${decision === 'APPROVE' ? 'GRANT MINISTERIAL APPROVAL' : 'REJECT'} the registration of "${app?.proposedName}". This is a final executive decision. Continue?`)) return;
        ...
        try {
            ...
            if (res.ok) {
                alert(`Ministerial ${decision === 'APPROVE' ? 'approval granted' : 'rejection recorded'} successfully.`);
                ...
                alert(err.error || 'Failed to process ministerial decision.');
            }
        } catch (e: any) {
    ...
    if (loading) return (
        <div className="p-8 flex items-center justify-center min-h-screen">
            <div className="text-center">
                ...
                <p className="text-gray-500 font-medium">Loading ministerial brief...</p>
            </div>
        </div>
    );
    ...
    if (error || !app) return (
        <div className="p-8 max-w-3xl mx-auto">
            <div className="glass-panel p-8 bg-rose-50 text-rose-700 text-center rounded-3xl">
                ...
                <Link href="/minister/dashboard" className="btn btn-primary">← Back to Cabinet</Link>
            </div>
        </div>
    );
    ...
    return (
        <div className="p-8 max-w-5xl mx-auto space-y-8 animate-fade-in-up">
            ...
            <div className="flex items-center gap-4">
                <Link href="/minister/dashboard" className="p-2 hover:bg-gray-100 rounded-xl transition-colors">
                    <svg className="w-5 h-5 text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" />
                    </svg>
                </Link>
                <div className="flex-1">
                    <div className="text-xs font-black text-indigo-600 uppercase tracking-widest mb-1">Ministerial Approval — Final Stage</div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="glass-panel p-6">
                    ...
                    <dl className="space-y-4">
                        ...
                        <div>
                            <dt className="text-xs font-bold text-gray-500 uppercase tracking-wider">Registration Fee</dt>
            ...
            {history.length > 0 && (
                <div className="glass-panel p-6">
                    <h3 className="text-lg font-bold text-gray-900 mb-4">📋 Full Review Trail</h3>
                    <div className="space-y-4">
                        {history.map((entry, idx) => (
                            <div key={entry.id} className="flex gap-4 relative">
                                ...
                                <div className="flex-shrink-0 w-8 h-8 rounded-full bg-indigo-100 border-2 border-indigo-400 flex items-center justify-center z-10">
            ...
            {/* Ministerial Decision Panel */}
            <div className="glass-panel p-8 bg-gradient-to-br from-indigo-950/5 to-purple-950/5 border-t-4 border-indigo-600 relative overflow-hidden">
                <div className="absolute -right-16 -bottom-16 w-64 h-64 bg-indigo-100 rounded-full blur-3xl opacity-30"></div>
                <div className="relative z-10">
                    <div className="flex items-center gap-3 mb-2">
                        <span className="text-3xl">🏛️</span>
                        <h3 className="text-2xl font-black text-gray-900">Ministerial Cabinet Decision</h3>
                    </div>
                    <p className="text-gray-500 text-sm mb-6">This is a final ministerial authority action. Your decision will be binding and permanently logged.</p>
                    ...
                    {isApproved ? (
                        <div className="bg-green-50 border-2 border-dashed border-green-400 rounded-2xl p-8 text-center">
                            ...
                            <h4 className="text-2xl font-black text-green-800 mb-2">Ministerial Approval Granted</h4>
                            <p className="text-green-700 font-medium">This society has received the highest level of governmental approval.</p>
                        ...
                        <div className="bg-red-50 border-2 border-dashed border-red-300 rounded-2xl p-8 text-center">
                            ...
                            <h4 className="text-2xl font-black text-red-800 mb-2">Registration Declined</h4>
                            <p className="text-red-700 font-medium">Ministerial decision to decline this registration is on record.</p>
                        </div>
                    ) : (
                        <>
                            <div className="mb-6">
                                <label className="block text-sm font-bold text-gray-700 mb-2 uppercase tracking-wider">
                                    Ministerial Statement <span className="text-red-500">*</span>
                                </label>
                                <textarea
                                    className="w-full rounded-2xl border border-gray-200 bg-white/70 p-4 text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent resize-none h-32"
                                    placeholder="Provide the official ministerial rationale for this decision. This statement becomes part of the permanent national record..."
                                    value={notes}
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <button
                                    ...
                                    disabled={processing}
                                    className="btn py-4 text-lg font-black bg-indigo-700 hover:bg-indigo-800 text-white shadow-xl shadow-indigo-200 disabled:opacity-40 disabled:cursor-not-allowed transition-all hover:-translate-y-0.5 active:scale-95"
                                >
                                    {processing ? 'Processing...' : '✅ Grant Ministerial Approval'}
                                </button>
                                <button
                            ...
                            <p className="mt-4 text-center text-xs text-gray-400 italic">
                                * As Minister&apos;s Delegate, your decision constitutes the final binding authority in the cooperative registration process.
                            </p>
                        </>
                    )}
                </div>
