...
export async function PATCH(request: NextRequest, { params }: { params: { id: string } }) {
    try {
        ...
        if (loan.status !== LoanStatus.PENDING) {
            return NextResponse.json({ error: `Cannot approve loan with status: ${loan.status}` }, { status: 400 });
        }
        ...
        return NextResponse.json({
            ...
            loan: { id: updated.id, loanNumber: updated.loanNumber, status: updated.status, approvalDate: updated.approvalDate, approvedBy: updated.approvedBy },
        });
    } catch (error: any) {
        console.error('Loan approval API error:', error);
        return NextResponse.json({ error: error.message || 'Failed to approve loan' }, { status: 500 });
    }
}
