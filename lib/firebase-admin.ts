...
function getAdminApp(): admin.app.App {
    ...
    if (!credentialsRaw) {
        throw new Error(
            ...
            'Please configure it with the Firebase Admin SDK service account JSON.'
        );
    }

    let serviceAccount;
    try {
        serviceAccount = JSON.parse(credentialsRaw);
    } catch (e) {
        throw new Error('Failed to parse FIREBASE_ADMIN_CREDENTIALS: invalid JSON');
    }
